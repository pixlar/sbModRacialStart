...
function init()
  ...
  quest.setParameter("uniformLocker", {type = "item", item = config.getParameter("uniformLocker", "protectoratelobbylockers2")})
  quest.setParameter("beamaxe", {type = "item", item = config.getParameter("beamaxe", "protectoratebeamaxe")})
  quest.setParameter("weaponChest", {type = "item", item = config.getParameter("weaponChest", "weaponchest")})
  quest.setIndicators({})

  setPortraits()

  self.startingMusicTimer = config.getParameter("startingMusicTime", 5)
...
function updateStage(dt)
  ...
  elseif self.missionStage == 7 then
    ...
    if player.hasItem(config.getParameter("starterweapon", "brokenprotectoratebroadsword")) then
      setStage(8)
    end
...
function hasUniform()
  local uniformChest = config.getParameter("uniformChest","protectoratechest")
  local uniformLegs = config.getParameter("uniformLegs","protectoratepants")
  return player.hasItem(uniformChest) and player.hasItem(uniformLegs)
end
...
function hasEquippedUniform()
  ...
  local uniformChest = config.getParameter("uniformChest","protectoratechest")
  local uniformLegs = config.getParameter("uniformLegs","protectoratepants")
  return ((chestItem and chestItem.name == uniformChest) or
          (chestCosmeticItem and chestCosmeticItem.name == uniformChest)) and
         ((legsItem and legsItem.name == uniformLegs) or
          (legsCosmeticItem and legsCosmeticItem.name == uniformLegs))
end
